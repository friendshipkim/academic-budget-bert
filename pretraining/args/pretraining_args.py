...
class PretrainScriptParamsArguments:
    ...
    load_checkpoint_id: Optional[str] = field(
        default=None,
        metadata={"help": "Checkpoint identifier to load from checkpoint path"},
    )
    ...
    validation_epochs: Optional[int] = field(
        default=10,
        metadata={"help": "Number of epochs between running validation evaluation"},
    )

    validation_epochs_begin: Optional[int] = field(
        default=1,
        metadata={
            "help": "Number of epochs between running validation evaluation in first stage"
        },
    )
    ...
    validation_epochs_end: Optional[int] = field(
        default=1,
        metadata={
            "help": "Number of epochs between running validation evaluation in last stage"
        },
    )

    validation_begin_proportion: Optional[float] = field(
        default=0.1,
        metadata={"help": "how long does the first stage of training last?"},
    )
    ...
    early_exit_time_marker: Optional[float] = field(
        default=100.0, metadata={"help": "Max hours for pre-training)"}
    )
    ...
    train_batch_size: Optional[int] = field(
        default=65536, metadata={"help": "train_batch_size"}
    )
    ...
    scale_cnt_limit: Optional[int] = field(
        default=100,
        metadata={
            "help": "The limit of the number of times the scale in the optimizer reached 1, in order to early stop."
        },
    )
    ...
    log_throughput_every: Optional[int] = field(
        default=20,
        metadata={
            "help": "How many steps should the throughput (im Samples/s) be logged."
        },
    )
